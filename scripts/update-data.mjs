...
function 以日期合併日線資料(rows = []) {
  const byDate = new Map();

  for (const row of rows ?? []) {
    const date = 正規化日期(row?.date);
...
function 建立官方個股日線列(市場個股資料, 市場資料日期, previousClose = null) {
  const date = 正規化日期(市場資料日期);
  const open = 取數字(市場個股資料?.開盤價);
  const high = 取數字(市場個股資料?.最高價);
  const low = 取數字(市場個股資料?.最低價);
  const close = 取數字(市場個股資料?.收盤價);

  if (!date || open === null || high === null || low === null || close === null) {
    return null;
  }

  const volume = 取數字(市場個股資料?.成交量);
  const turnover = 取數字(市場個股資料?.成交值);
  const transactions = 取數字(市場個股資料?.成交筆數);
  const rawChange = 取數字(市場個股資料?.漲跌值);
...
function 合併最新官方日線(rows = [], 市場個股資料, 市場資料日期) {
  const existingRows = (rows ?? []).filter((item) => 正規化日期(item?.date));
  const previousClose = existingRows.at(-1)?.close ?? null;
  const latestOfficialRow = 建立官方個股日線列(市場個股資料, 市場資料日期, previousClose);

  if (!latestOfficialRow) {
    return 補上日線漲跌資料(existingRows);
  }

  return 補上日線漲跌資料(以日期合併日線資料([...existingRows, latestOfficialRow]));
}
...
async function 抓取TWSE歷史日線(code, monthCount = 3) {
  const now = new Date();
  const months = [];

  for (let offset = monthCount - 1; offset >= 0; offset -= 1) {
    const date = new Date(now.getFullYear(), now.getMonth() - offset, 1);
    months.push(轉日期查詢字串(date).slice(0, 6) + '01');
  }
  ...
  return 補上日線漲跌資料(以日期合併日線資料(rows));
}
...
async function 抓取證券歷史日線(code, monthCount = 6, options = {}) {
  const {
    市場個股資料 = null,
    市場資料日期 = null,
    舊歷史資料 = [],
  ...
  try {
    const yahooRows = await 抓取Yahoo歷史日線([`${code}.TW`, `${code}.TWO`], monthCount);
    return 合併最新官方日線(yahooRows, 市場個股資料, 市場資料日期);
  } catch (yahooError) {
    let fallbackRows = [];

    if (市場個股資料) {
      try {
        fallbackRows = await 抓取TWSE歷史日線(code, Math.max(2, Math.min(monthCount, 3)));
      } catch {
        fallbackRows = [];
      }
    }

    const mergedRows = 合併最新官方日線(
...
function 計算收盤簡單均線(rows, index, period) {
  if (index + 1 < period) {
    return null;
  }
...
function 建立上市股票資料(item) {
  ...
  return {
    代號: 壓縮文字(item.Code),
    名稱: 壓縮文字(item.Name),
    開盤價: 取數字(item.OpeningPrice),
    最高價: 取數字(item.HighestPrice),
    最低價: 取數字(item.LowestPrice),
    收盤價,
    漲跌值,
    漲跌幅: 計算漲跌幅(收盤價, 漲跌值),
...
function 建立加權指數權重索引(全部個股 = [], 公司概況索引 = new Map()) {
  const rows = (全部個股 ?? [])
    .map((item) => {
      const issuedShares = 取數字(公司概況索引.get(item.代號)?.已發行股數);
      const close = 取數字(item.收盤價);

      if (!是否一般股票(item.代號) || issuedShares === null || issuedShares <= 0 || close === null || close <= 0) {
        return null;
      }
...
function 建立單股重壓名單(successes, { taiexWeightMap = new Map() } = {}) {
  const map = new Map();
  ...
  for (const result of successes) {
    for (const holding of result.snapshot?.holdings ?? []) {
      ...
      const weight = 取數字(holding.weight);
...
function 建立ETF重疊分析(successes, { 全部個股 = [], 公司概況索引 = new Map() } = {}) {
  ...
  const 加權指數權重索引 = 建立加權指數權重索引(全部個股, 公司概況索引);
  const 單股重壓 = 建立單股重壓名單(successes, { taiexWeightMap: 加權指數權重索引 });
...
function 建立產業估值股票池(全部個股, 公司概況索引, 評價索引) {
...
function 計算平均成交值(歷史資料 = []) {
  const values = (歷史資料 ?? [])
    .slice(-20)
    .map((item) => {
      const turnover = 取數字(item?.turnover);
      if (turnover !== null && turnover > 0) return turnover;

      const close = 取數字(item?.close);
      const volume = 取數字(item?.volume);
...
function 建立財報索引(財報日曆資料) {
  const map = new Map();

  for (const item of 財報日曆資料?.items ?? []) {
    const code = 壓縮文字(item?.code);
    if (!code) continue;
    if (!map.has(code)) {
      map.set(code, []);
    }
...
function 補強個股摘要清單({
  ...
  const 產業估值索引 = buildIndustryValuationIndex(建立產業估值股票池(全部個股, 公司概況索引, 評價索引));
  const 財報索引 = 建立財報索引(財報日曆資料);
  const 內部人索引 = 建立內部人索引(內部人資料);
  ...
  return 摘要清單.map((summary) => {
    ...
    return {
      ...
      hasMarginSurge:
        (取數字(融資融券?.marginUsage) ?? 0) >= 40 || (取數字(融資融券?.marginChange) ?? 0) >= 800000,
...
async function 寫入個股明細資料(候選清單, 日資料清單, tdcc索引, 財務索引, 其他索引) {
  ...
  for (const item of 候選清單) {
    ...
    try {
      ...
      const 歷史資料 = await 抓取證券歷史日線(item.code, 12, {
        市場個股資料,
      ...
      try {
        盤中走勢 = await 抓取Yahoo盤中分時([`${item.code}.TW`, `${item.code}.TWO`]);
      } catch (error) {
...
async function main() {
  ...
  try {
    const 即時個股索引 = await 批次抓取MIS個股快照(個股候選清單.map((item) => item.code));
    市場個股索引 = new Map([
      ...市場個股索引.entries(),
  ...
  await 寫入JSON(path.join(資料目錄, 'stocks', 'index.json'), 強化個股摘要清單);
  ...
  await 寫入JSON(path.join(財報日曆資料目錄, 'product-events.json'), 產品事件資料);
  await 寫入JSON(path.join(內部人資料目錄, 'holdings.json'), 內部人持股資料);
